...
Formeln:
  Kontrast (Strehl):
    W_PtV (paraxial)  = D⁴ / (2048·f³·λ)          [Rutten/van Venrooij, FigureXP-kompatibel]
    ...
    Strehl (exakt)    = |⟨exp(i·2π·W(ρ))⟩|²         [Pupillenintegral am Paraxialfokus,
                        W(ρ) = Wp·ρ⁴, nur Piston;   FigureXP-kompatibel]
...
def _golden_section(f, a: float, b: float, tol: float = 1e-8) -> float:
    """Golden-Section-Suche fuer Minimum von f auf [a, b].
    Ersetzt scipy minimize_scalar -- keine scipy-Abhaengigkeit noetig.
    Konvergiert in <=200 Iterationen auf tol=1e-8.
    """
...
def _wellenfronten(D_mm: float, f_mm: float, lam_nm: float):
    ...
    Wp  = D^4/(2048*f^3*lam) -- PtV-Wellenfrontfehler am paraxialen Fokus
                                 [Rutten/van Venrooij; FigureXP-kompatibel]
                                 Faktor 2048 = paraxiale Seidel-Naeherung fuer Reflexion.
                                 (Frueher 1024; Aenderung V6.3.0 fuer FigureXP-Konsistenz.)
    Wb  = Wp/4               -- PtV am besten Fokus [Sacek 4.7.2]
    Wrms= Wb/(1.5*sqrt(5))   -- RMS fuer reine sphärische Aberration [Sacek 4.7.2]
    S   = exp(-(2pi*Wrms)^2) -- Marechal-Naeherung, gueltig fuer S > ~0.6
    """
    lam_mm = lam_nm * 1e-6
    Wp   = D_mm**4 / (2048.0 * f_mm**3 * lam_mm)
    Wb   = Wp / 4.0
...
def _strehl_exakt(Wb: float, n: int = 2000) -> float:
    """Exakter Strehl via Pupillenintegral am Paraxialfokus.

    S = |<exp(i*2pi*W(rho))>|^2   mit  W(rho) = 4*Wb*rho^4
    Wb = PtV-Wellenfrontfehler am besten Fokus = Wp/4
    Wp = D^4/(2048*f^3*lam)  (paraxialer PtV, Rutten-Konvention)
    => W(rho) = Wp*rho^4  [Paraxialfokus, nur Piston subtrahiert]
    FigureXP-kompatibel: wertet am Paraxialfokus aus, kein Defokus-Term.
    (Frueher: W = 4*Wb*(rho^4 - rho^2), Aenderung V6.3.0)
    Marechal-Naeherung versagt fuer Wb > 0.08 lambda (ca. f/N < 9).
    """
    _trapz = getattr(np, "trapezoid", getattr(np, "trapz", None))
    rho   = np.linspace(0.0, 1.0, n)
    W     = 4.0 * Wb * rho**4                   # Paraxialfokus, kein (rho^4 - rho^2)
    W_m   = float(_trapz(W * rho, rho) / 0.5)   # flaechengewichteter Mittelwert (Piston)
    Wc    = W - W_m
...
def foucault_wellenfront(r_zonen: np.ndarray, df_zonen: np.ndarray,
    ...
    Wellenfrontfehler [in Einheiten von lambda]:
        W(r) = Delta_l(r) * r^2 / (8 * f^2 * lambda)
    Kontrolle 200/1000: Kugel -> S=0.1232 (FigureXP-kompatibel) ✓
    """
    lam_mm      = lam_nm * 1e-6
    ...
    delta_l     = dl_para_rel - df_zonen                 # Abweichung: Soll-Ist
                                                         ...
                                                         # <0 = ueberkorrekt  (Ist zu gross)
    # Herleitung (Sacek): W(r) = Delta_l(r) * r^2 / (8*f^2*lambda)
    # Faktor 8 = korrekter doppelter Weg beim Spiegel (Reflexion).
    # (Frueher Faktor 16; Aenderung V6.3.0 fuer FigureXP-Konsistenz.)
    # Kontrolle: Kugel 200/f5 -> S=0.1232 (FigureXP) ✓
...
def foucault_kennzahlen(W_zonen: np.ndarray, r_zonen: np.ndarray,
    ...
    """PtV, RMS, Strehl aus Zonenwellenfronten.

    Zwei Modi (use_direct):

    False -- Polynom-Fit (Standard, glatte Flaeche):
        Passt W(rho) = a4*rho^4 + a2*rho^2 an die Zonenwerte an.
        Korrekt fuer reine sphaerische Aberration.
        Zonenfehler (lokale Ringfehler) werden weggemittelt:
        Strehl wird dann zu optimistisch.

    True -- Hybrid-Modus (Zonenfehler):
        Gleiche Polynom-Grundform, aber Abweichungen der Messwerte
        vom Polynom (Residuen) werden als stueckweise konstante
        Ringzonen-Fehler auf die Wellenfront aufgesattelt.
        Erkennt echte Zonenfehler ohne die Grundform zu verfaelschen.
        Bei glattem Spiegel (Residuen=0): identisch mit Polynom-Modus.

    Beide Modi: Defokus-Optimierung (bester Fokus), Strehl via
    Pupillenintegral, keine scipy-Abhaengigkeit.

    Rueckgabe-Dict -- alle Schluessel immer vorhanden:
        W_zonen, W_c_zonen, W_rms, W_ptv, Wb_fit
        a4, a2, fit_residuum_rms
        S_mar, S_exakt, Deff_k
        Glas_f, Glas_zonen, Glas_ptv_nm, Glas_volumen_mm3, W_min, lam_nm
        rho_f, W_best, W_c
    """
    _trapz = getattr(np, "trapezoid", getattr(np, "trapz", None))
    ...
    # Flaechengewichte der Zonen (Kreisringflaechen, normiert)
    ...
    # Defokus-Optimierung (bester Fokus)
    def _wrms_sq(d):
        Wt = W_f + d * rho_f**2
    ...
    if lam_nm is not None:
        ...
        Glas_volumen_mm3 = float(
            2 * math.pi * R**2 * _trapz(Glas_f * rho_f, rho_f))
...
def mtf_sph_rel(f: float, Wb: float, n: int = 80,
                paraxial: bool = True) -> float:
    """Relative MTF eines sphaerischen Spiegels (normiert: MTF(0)=1).

    OTF-Faltungsintegral [Sacek 4.7]:
      MTF(f) = |int exp(i*2pi*DeltaW(x)) h(x) dx| / int h(x) dx
    mit DeltaW(x) = W(x+f/2) - W(x-f/2)

    Wellenfrontform (V6.3.0: Standard = Paraxialfokus, FigureXP-kompatibel):
      parax. Fokus (paraxial=True,  Standard): W(rho) = Wp*rho^4
      best focus   (paraxial=False):           W(rho) = Wp*(rho^4 - rho^2)

    Amplitude Wp = 4*Wb  (Wp_paraxial = 4*Wb, da Wb = Wp/4):
      W(rho=1) = Wp = 4*Wb am Paraxialfokus.
      (Frueher Wp = 8*Wb; Aenderung V6.3.0 zusammen mit Wp-Halbierung in _wellenfronten)

    f  = normierte Ortsfrequenz (0..1)
    Wb = W_PtV_bestfocus  (= Wp_paraxial/4, nach V6.3.0: Wp_paraxial = D^4/(2048*f^3*lam))
    """
    if f <= 0: return 1.0
    if f >= 1: return 0.0
    lim = math.sqrt(max(0.0, 1.0 - (f/2)**2))
    re = im = norm = 0.0
    Wp = Wb * 4  # Wp_paraxial = 4*Wb  (V6.3.0: war 8*Wb)
    for i in range(n):
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        for attr, tab_key in tab_attrs:
            ...
            if attr == "_tab_mtf":
                ...
                # Zeile 2: Auswertungs-Modus + Status
                ctrl2 = tk.Frame(frm, bg=BG)
                ctrl2.pack(side="top", fill="x", padx=8, pady=(0, 2))

                tk.Label(ctrl2, text="Auswertung:", bg=BG, fg="#555",
                         font=("Helvetica", 9)).pack(side="left")
                self._foucault_use_direct = tk.BooleanVar(value=False)
                tk.Radiobutton(ctrl2,
                               text="Polynom-Fit  (glatte Flaeche)",
                ...
                self._foucault_status = tk.Label(ctrl2, text="", bg=BG,
    ...
    def _foucault_aktualisieren(self):
        ...
        try:
            ...
            n          = self._foucault_n.get()
            # _foucault_use_direct wird erst beim Tab-Aufbau erstellt;
            # Default False (Polynom) wenn Tab noch nicht geoeffnet wurde.
            if not hasattr(self, "_foucault_use_direct"):
                self._foucault_use_direct = tk.BooleanVar(value=False)
            use_direct = self._foucault_use_direct.get()

            r_zon  = foucault_zonen(D, n)
            df_arr = np.array([v.get() for v in self._foucault_df_vars])
            df_para_abs = r_zon**2 / (2.0 * 2.0 * f)
            ...
            for i, dv in enumerate(abw):
                for w in self._foucault_eingabe_frm.grid_slaves(row=i+1, column=5):
                    col = "#2e7d32" if abs(dv) < 0.01 else                           "#e65100" if abs(dv) < 0.1  else "#c62828"
            ...
            fehlend = np.where(dl_soll_arr > 0.001,
                               (dl_soll_arr - df_arr) / dl_soll_arr, 0.0)
            kugel_zonen = [f"Z{i+1}" for i, feh in enumerate(fehlend)
                           if feh > 0.80 and i > 0]
            if kugel_zonen:
                warn += (f"  !! {", ".join(kugel_zonen)} noch"
                         f" auf Kugelnaehe -- S nicht repraesentativ!")
                s_col = "#c62828"
            status = (
                f"[{modus}]  "
                f"S={kz['S_exakt']:.4f}   "
                f"Wrms={kz['W_rms']:.4f}lambda   "
                f"WPtV={kz['W_ptv']:.4f}lambda   "
                f"Abtrag={kz['Glas_ptv_nm']:.1f}nm   "
                f"V={kz['Glas_volumen_mm3']:.1f}mm3   "
                f"Deff={kz['Deff_k']:.1f}mm   "
                f"{'OK Rayleigh' if rayleigh_ok else 'FAIL Rayleigh'}"
                f"{warn}"
            )
    ...
    def _diagramm_foucault(self, D, f, lam, r_zon, df_arr, kz, use_direct=False):
